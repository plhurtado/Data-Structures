...
@return true si la entrada se ha podido insertar con éxito, esto es, no existe un meteorito con igual nombre en el diccionario. False en caso contrario
 @post Si e no esta en el diccionario se inserta, el size() sera incrementado en 1.
*/
...
private:
  ...
  /** @brief Chequea el Invariante de la representacion
      @return true si el invariante es correcto, falso en caso contrario //Estas son las reglas que tienes que cumplir cada version
*/
  bool cheq_rep( ) const;
  pair <int, bool> findPosition (const nombreM &nombre) const;
